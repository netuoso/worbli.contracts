...
namespace eosiosystem {
   ...
   void system_contract::buyrambytes( name payer, name receiver, uint32_t bytes ) {
      check( payer == "worbli.admin"_n || payer == _self , "only worbli.admin or eosio can purchase RAM" );
  ...
  void system_contract::buyram( name payer, name receiver, asset quant ) {
      require_auth( payer );
      check( payer == "worbli.admin"_n || _self , "only worbli.admin or eosio can purchase RAM" );
