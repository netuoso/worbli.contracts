...
class [[eosio::contract]] playground : public eosio::contract {
public:
  playground( name self, name code, datastream<const char*> ds )
  ...
  struct [[eosio::table("testdata")]] testdata {
    ...
    std::vector<name> vect_field;

    uint64_t primary_key()const { return name_field.value; }
    
    EOSLIB_SERIALIZE( testdata, (name_field)(str_field)(int16_field)(vect_field) )
  };
  ...
  [[eosio::action]]
  void insrow(name name_field, string str_field, uint16_t int16_field, std::vector<name> vect_field)
  {
...
EOSIO_DISPATCH( playground, (insrow)(delrow) )
