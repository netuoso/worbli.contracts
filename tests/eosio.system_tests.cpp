...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.0000"), get_balance( "alice1111111" ) + refund["ram_amount"].as<asset>());
   total = get_total_stake( "alice1111111" );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100001000.0000"), get_balance( "alice1111111" ) + refund["ram_amount"].as<asset>() );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("90001000.0000"), get_balance( "alice1111111" ) );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100001000.6144"), get_balance( "alice1111111" ) + refund["ram_amount"].as<asset>());
   ...
   refund = get_refund_request( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100000440.6144"), get_balance( "alice1111111" ) + refund["ram_amount"].as<asset>());
   ...
   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", bought_bytes ) );
   refund = get_refund_request( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100001000.3938"), get_balance( "alice1111111" ) + refund["ram_amount"].as<asset>());
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("49401000.3938"), get_balance( "alice1111111" ));
   ...
   refund = get_refund_request( "alice1111111" );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("100000998.6530"), get_balance( "alice1111111" ) + refund["ram_amount"].as<asset>());
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_sym::from_string("2.0000"), core_sym::from_string("0.0002") ) );
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   create_account_with_resources( N(defproducera), config::system_account_name, core_sym::from_string("1000.0000"), false, large_asset, large_asset );
   create_account_with_resources( N(defproducerb), config::system_account_name, core_sym::from_string("1000.0000"), false, large_asset, large_asset );
   create_account_with_resources( N(defproducerc), config::system_account_name, core_sym::from_string("1000.0000"), false, large_asset, large_asset );

   create_account_with_resources( N(producvotera), config::system_account_name, core_sym::from_string("1000.0000"), false, large_asset, large_asset );
   create_account_with_resources( N(producvoterb), config::system_account_name, core_sym::from_string("1000.0000"), false, large_asset, large_asset );
...

   {
      ...
      const uint64_t initial_distribution_time = microseconds_since_epoch_of_iso_string(initial_global_state["last_inflation_distribution"]);
      ...
      const auto global_state          = get_global_state();
      const uint64_t distribution_time = microseconds_since_epoch_of_iso_string(global_state["last_inflation_distribution"]);
   ...
   // test change in network usage level to 31%
   {
      const auto     initial_global_state      = get_global_state();
      const uint64_t initial_distribution_time = microseconds_since_epoch_of_iso_string(initial_global_state["last_inflation_distribution"]);
      const int64_t  initial_savings           = get_balance(N(eosio.saving)).get_amount();
   ...
   // test for 7 years
   {
      ...
      produce_blocks(1);
      const asset   supply  = get_token_supply();
      std::cout << "7 year token supply: " << supply.get_amount() << "\n";
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      BOOST_REQUIRE (0 ==  microseconds_since_epoch_of_iso_string(proda["last_claim_time"]) && 
               0 == microseconds_since_epoch_of_iso_string(prodz["last_claim_time"]));
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("assertion failure with message: producer not found"),
                           push_action( prod_name, N(unregprod), mvo() ("producer",  prod_name ) ));
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "eosio", "defproducer3", core_sym::from_string("10000.0000"), "eosio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "defproducer3", "defproducer3", core_sym::from_string("5000.0000"), core_sym::from_string("5000.0000") ) );
   BOOST_REQUIRE_EQUAL(success(),
                      push_action("defproducer3", N(unregprod), mvo()("producer", "defproducer3")));
      BOOST_REQUIRE_EQUAL(success(),
                      push_action("defproducer3", N(unregprod), mvo()("producer", "defproducer3")));
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_rex, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( get_balance( N(eosio.rex) ),       cur_rex_balance );
   ...
   BOOST_REQUIRE_EQUAL( get_balance( N(eosio.rex) ),      cur_rex_balance + core_sym::from_string("5.0000") );
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, eosio_system_tester ) try {
   ...
   transfer( N(eosio), N(worbli.admin), core_sym::from_string("34043.0000") );

   auto error_msg = stake( N(worbli.admin), N(eosio), core_sym::from_string("10.0000"), core_sym::from_string("10.0000") );
   auto semicolon_pos = error_msg.find(';');
   ...
   // std::cout << "bytes needed: " << ram_bytes_needed << std::endl;
   // 68 bytes per WBI

   auto alice_original_balance = get_balance( N(worbli.admin) );
   
   BOOST_REQUIRE_EQUAL( success(), buyrambytes( N(worbli.admin), N(eosio), static_cast<uint32_t>(ram_bytes_needed) ) );

   auto tokens_paid_for_ram = alice_original_balance - get_balance( N(worbli.admin) );
   ...
   REQUIRE_MATCHING_OBJECT( total_res, mvo()
      ...
      ("ram_stake", core_sym::from_string("34042.4264"))
